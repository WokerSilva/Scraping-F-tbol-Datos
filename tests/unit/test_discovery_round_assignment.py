...
class _Parser:
    def parse(self, html: str) -> dict:
        ...
        return {"matches": [{"source_match_id": marker, "url": f"/{marker}"}]}
...
def test_discovery_round_assignment_per_match() -> None:
    ...
    rows = use_case.execute(competition_slug="clausura_mexico", year=2026, dry_run=True, persist=False, browser=True)
