...
class _Parser:
    def parse(self, html: str) -> dict:
        return {"matches": [{"source_match_id": "mx1", "url": "/partido/mx1"}]}
...
def test_discovery_uses_browser_fallback_on_406() -> None:
    ...
    rows = use_case.execute(competition_slug="clausura_mexico", year=2026, dry_run=True, persist=False, browser=False)
    assert rows
